...
/*
  ...
  Main features:
    ...
    - Stable-press confirmation for reliable button taps
    - Non-overlapping keypad hit detection (fixes COS/SIN overlap)
    - On-screen expression keypad
    - Parser for +, -, *, /, ^, parentheses, x, pi, e
    - Functions: sin, cos, tan, sqrt, log, exp, abs
    - Graph plotting, four-direction pan, and zoom
...
// Require a few stable mapped samples before accepting a key press.
// This avoids using the inaccurate first contact sample from a resistive panel.
const uint8_t PRESS_CONFIRM_SAMPLES = 3;
const int16_t PRESS_MAX_SPREAD_PIXELS = 20;
// Serial diagnostics removed to fit safely in Arduino UNO flash.
...
const uint32_t CAL_MAGIC = 0x47524334UL;  // "GRC4" forces fresh calibration
...
uint8_t pressSampleCount = 0;
int16_t pressSampleX[PRESS_CONFIRM_SAMPLES];
int16_t pressSampleY[PRESS_CONFIRM_SAMPLES];
...
void runCalibrationWizard() {
  ...
  tft.print("Hold each cross");
  tft.setCursor(28, 153);
  tft.print("Use plastic stylus");
  delay(1200);
...
const char *toolbarLabels[8] = {"EDIT", "+", "-", "<", ">", "^", "v", "HOME"};
...
void panGraph(float xFraction, float yFraction) {
  float shiftX = (viewXMax - viewXMin) * xFraction;
  float shiftY = (viewYMax - viewYMin) * yFraction;
  viewXMin += shiftX;
  viewXMax += shiftX;
  viewYMin += shiftY;
  viewYMax += shiftY;
...
void handleMainKey(const char *label) {
  ...
  if (strcmp(label, "PLOT") == 0) {
    if (expressionLength == 0 || !validateExpression()) {
      showEditMessage("Invalid", COLOR_RED);
      return;
    }
  ...
  if (!appendText(label)) {
    showEditMessage("Full", COLOR_RED);
  } else {
    drawExpressionBox();
  }
}
...
void handleEditTouch(int16_t x, int16_t y) {
  ...
  if (x < 0 || x >= tft.width() ||
      y < keypadTop || y >= tft.height()) return;

  // Convert the touch directly to one non-overlapping keypad cell.
  // This is more reliable than scanning expanded rectangles.
  uint8_t column = (uint8_t)(((int32_t)x * columns) / tft.width());
  uint8_t row = (uint8_t)(((int32_t)(y - keypadTop) * rows) /
                           (tft.height() - keypadTop));
...
void handleGraphTouch(int16_t x, int16_t y) {
  ...
  if (x < 0 || x >= tft.width() ||
      y < tft.height() - GRAPH_TOOLBAR_H || y >= tft.height()) return;

  uint8_t i = (uint8_t)(((int32_t)x * toolbarCount) / tft.width());
  if (i >= toolbarCount) i = toolbarCount - 1;
...
bool collectStablePressPoint(
  int16_t x, int16_t y, int16_t &stableX, int16_t &stableY
) {
  if (pressSampleCount < PRESS_CONFIRM_SAMPLES) {
    pressSampleX[pressSampleCount] = x;
    pressSampleY[pressSampleCount] = y;
    ++pressSampleCount;
  }

  if (pressSampleCount < PRESS_CONFIRM_SAMPLES) return false;

  int16_t minX = pressSampleX[0];
  int16_t maxX = pressSampleX[0];
  int16_t minY = pressSampleY[0];
  int16_t maxY = pressSampleY[0];

  for (uint8_t i = 1; i < PRESS_CONFIRM_SAMPLES; ++i) {
    if (pressSampleX[i] < minX) minX = pressSampleX[i];
    if (pressSampleX[i] > maxX) maxX = pressSampleX[i];
    if (pressSampleY[i] < minY) minY = pressSampleY[i];
    if (pressSampleY[i] > maxY) maxY = pressSampleY[i];
  }

  if ((maxX - minX) > PRESS_MAX_SPREAD_PIXELS ||
      (maxY - minY) > PRESS_MAX_SPREAD_PIXELS) {
    // The stylus was still settling. Start again using the newest point.
    int16_t newestX = pressSampleX[PRESS_CONFIRM_SAMPLES - 1];
    int16_t newestY = pressSampleY[PRESS_CONFIRM_SAMPLES - 1];
    pressSampleX[0] = newestX;
    pressSampleY[0] = newestY;
    pressSampleCount = 1;
    return false;
  }

  stableX = median5(pressSampleX, PRESS_CONFIRM_SAMPLES);
  stableY = median5(pressSampleY, PRESS_CONFIRM_SAMPLES);
  return true;
}
...
bool startupTouchRequestsCalibration() {
  ...
  tft.setTextColor(COLOR_WHITE, COLOR_BLACK);
  tft.setCursor(8, 120);
  tft.print("Hold screen: calibrate");
  tft.setCursor(8, 138);
  tft.print("Starting...");
...
void loop() {
  ...
  if (touchDown) {
    ...
    // Do not act on the first contact sample. Wait for three mapped points
    // that agree with one another, then use their median position.
    if (!pressHandled && millis() - lastTouchReleaseMs > 70UL) {
      int16_t stableX, stableY;
      if (collectStablePressPoint(x, y, stableX, stableY)) {
        pressHandled = true;

        if (screenMode == MODE_GRAPH) handleGraphTouch(stableX, stableY);
        else handleEditTouch(stableX, stableY);
